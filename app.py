...
st.markdown("""
    <style>
    /* Expander Header Visibility Fix - Blue background for contrast */
...
def get_csv_urls(selected_envs):
    ...
    # Universal equipment (Included in ALL locations)
    files = ["general-loop-band.csv", "general-mini-bands.csv"]
    
    # Weight Room Specific mapping
...
with st.sidebar:
    ...
    num = st.slider("Max Exercises", 5, 20, 12)
...
if st.session_state.current_session and not st.session_state.workout_finished:
    for i, drill in enumerate(st.session_state.current_session):
        with st.expander(f"{drill['ex']}", expanded=(i==0)):
            ...
            with col2:
                ...
                m3.markdown(f"<p class='metric-label'>Goal Timer</p><p class='metric-value'>{drill['timer']}s</p>", unsafe_allow_html=True)
                
                st.info(f"🏅 **HS Standard:** {drill['hs']} | 🎓 **College:** {drill['coll']}")
    ...
    st.info("👋 Select locations in the sidebar and click 'Build Workout' to begin.")
